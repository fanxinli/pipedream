...
if __name__ == "__main__":
    ...
    # run using Python3.
    module_path = '%s' % configurations[MODULE]
    if not args.quiet:
    ...
    main_runtime_filename = 'bert_with_runtime.py'
    ...
    if args.launch_single_container:
        ...
        for node_rank, (node_ip, workers) in \
            ...
            docker_cmd = 'nvidia-docker run -it --rm %(mount_directories)s ' \
                         '--net=host ' \
                         '--shm-size 16g %(container)s /bin/bash -c' % {
                "container": configurations[CONTAINER],
                "mount_directories": "-v $(dirname $PWD):/workspace -v ${PWD}:/workspace/bert"
            }
